...
This section presents the results from estimating the macroeconomic factor model, with \ref{Fig10}-\ref{Fig12} displaying the estimated **$\alpha$'s** and **$\beta$'s** for each industry, where $\beta$ represents each assets sensitivity to the factors, and alpha the constant (excess) returns.
